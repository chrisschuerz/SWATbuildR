...
## Basin boundary
bound <- sf::read_sf(bound_path)
bound <- bound['geometry']
set_proj_crs(bound, data_path)
check_polygon_topology(layer     = bound,
                       data_path =  data_path,
                       label     = 'basin',
                       n_feat    = 1,
                       checks    = c(F,T,T,T,F,F,F,F))

## Land layer
land <- sf::read_sf(land_path)
land <- check_layer_attributes(land, type_to_lower = FALSE)
land <- check_project_crs(layer      = land,
                          data_path  =  data_path,
...
check_polygon_topology(layer     = land,
                       data_path =  data_path,
                       label     = 'land',
                       area_fct  =  0.05,
                       cvrg_frc  = 99.9,
...
if(!is.null(channel_path)) {
  channel <- sf::read_sf(channel_path)
  channel <- check_layer_attributes(channel, type_to_lower = TRUE)
  channel <- check_project_crs(layer      = channel,
                               data_path  =  data_path,
                               proj_layer = project_layer,
                               label      = 'channel',
                               type       = 'vector')
  ...
  ## Check function saves layer into data/vector after all checks were successful
  check_line_topology(layer      = channel,
                      data_path  = data_path,
                      label      = 'channel',
                      length_fct = 0.05,
                      can_cross  = TRUE)
...
dem <- check_project_crs(layer      = dem,
                         data_path  = data_path,
                         proj_layer = project_layer,
                         label      = 'dem',
                         type       = 'raster')

check_raster_coverage(rst       = dem,
                      vct_layer = 'land',
                      data_path = data_path,
                      label     = 'dem',
                      cov_frc   = 0.95)
...
soil <- terra::rast(soil_layer_path)
soil <- check_project_crs(layer      = soil,
                          data_path  = data_path,
                          proj_layer = project_layer,
                          label      = 'soil',
                          type       = 'raster',
...
check_raster_coverage(rst       = soil,
                      vct_layer = 'hru',
                      data_path = data_path,
                      label     = 'soil',
                      cov_frc   = 0.75)
