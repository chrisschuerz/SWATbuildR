#' Check the water object connectivity.
#'
...
#' @param id_cha_out Integer value, if set the respective channel unit is
#'   considered as the end unit of the water object network
#' @param id_res_out Integer value, if set the respective reservoir unit is
#'   considered as the end unit of the water object network
#'
...
#' Prepare channel and reservoir objects for the generation of the water object
#' network.
#'
#' The reservoir and the channel layer attribute tables are restructured to have
#' an id from 1 to nrow(.) and keeping the initial object ids as 'id_sel', also
#' object names and empty gis_id are added.
#'
#' @param data_path Character string, path to the project data folder
#'
#' @returns A list with channel and reserwoir object tables.
#'
#' @importFrom dplyr mutate rename select %>%
#' @importFrom lwgeom st_split
#' @importFrom sf read_sf st_collection_extract st_buffer st_within
#'
#' @keywords internal
#'
...
#' Calculate the water object connectivity.
#'
#' The routine identifies the connections between channel and reservoir objects.
#' It propagates from the catchment outlet object backwards to link all objects
#' of the network. The modeler has to provide either the id of the final channel
#' or the final reservoir object (initial ids of the input layers channel or
#' land). The routine returns a list with the 'chandeg_con_out' and the
...
#'   considered as the end unit of the water object network
#'
#' @returns Writes the following tables into the intermediate data base
#'   data/tables.sqlite:
#'    - cha.chandeg_con_out
#'    - res.reservoir_con_out
#'
#' @importFrom DBI dbConnect dbDisconnect dbReadTable dbWriteTable
#' @importFrom dplyr bind_rows %>%
#' @importFrom purrr map map2_df
#' @importFrom RSQLite SQLite
#' @importFrom sf read_sf st_buffer st_intersects
...
#'
#' @importFrom DBI dbConnect dbDisconnect dbReadTable dbWriteTable
#' @importFrom dplyr ends_with mutate select %>%
#' @importFrom purrr map map_df map2 set_names
#' @importFrom RSQLite SQLite
#'
#' @export
#'
...
#'
#' The connectivity link tables are translated into the SWAT input table format
#' of *_con_out tables.
#'
