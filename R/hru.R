...
#'
#' @returns Two shape file layers in the folcer vector:
#'   - hru.shp contains all land polygons
#'   - res.shp contains all 'watr' polygons which will be reservoirs.
#'
...
#' @param data_path Path of the project data folder
#'
#' @returns The function writes the following tables into the intermediate
#'   data base:
#'   - landuse.ids
#'   - landuse.landuse_lum
#'
#' @importFrom DBI dbConnect dbDisconnect dbReadTable dbWriteTable
#' @importFrom dplyr group_by mutate rename select summarize %>%
...
#' leaching potential and the runoff potential of an HRU.
#'
#' The leaching potential and the runoff potential can be identified based on the
